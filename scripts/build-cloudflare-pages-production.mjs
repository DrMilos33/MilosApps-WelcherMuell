...
const sourceBranch = process.env.WASTE_GUIDE_PRODUCTION_SOURCE_BRANCH || "codex/waste-guide-production-launch";
...
offlineWorker = replaceExactly(
  ...
  'const CACHE_NAME = "waste-guide-production-2026-08-03-search-v6";',
...
storageInventory.reviewedAt = "2026-08-04";
...
cacheEntry.identifier = "waste-guide-production-2026-08-03-search-v6";
storageInventory.externalServices = storageInventory.externalServices.map((entry) =>
  entry.service === "GitHub Pages"
    ? { ...entry, service: "Cloudflare Pages", purpose: "Production-App und alle redaktionellen Daten als Same-Origin-Dateien ausliefern" }
    : entry
);
...
const csp = "default-src 'self'; script-src 'self'; style-src 'self'; img-src 'self'; connect-src 'self'; font-src 'self'; worker-src 'self'; manifest-src 'self'; object-src 'none'; media-src 'none'; base-uri 'none'; form-action 'self'; frame-ancestors 'none'; upgrade-insecure-requests";
const headers = `/*
